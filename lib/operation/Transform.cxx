    ...
    for (std::size_t nRead = 1; fileSource->FileStatusOK();
      ...
      try {
        ...
        if ((!limitedRecPerMolecule ||
             (limitedRecPerMolecule &&
              numRecPerMolecule[moleculeName] < maxNumRecPerMolecule)) &&
            (!checkName || (checkName && moleculeName == name)) &&
            (!checkData ||
             (checkData &&
              model->GetDataValue(dataFieldName).GetString() == dataValue))) {
          if (numRecPerMolecule.count(moleculeName) == 0)
            numRecPerMolecule[moleculeName] = 1;
          else
            numRecPerMolecule[moleculeName]++;
